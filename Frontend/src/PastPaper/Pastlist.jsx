...
const Pastlist = () => {
    ...
    useEffect(() => {
        const getcourse = async () => {
            try {
                const res = await axios.get(`https://notesflix-s5ki.vercel.app//Pastlist`);
                        ...
                        <div className="w-[90vw] ">
                            {CourseData.data && CourseData.data.map((i) => (
                                <div key={i.sem} className="m-8 w-full">
                                    ...
                                    <h1 className="text-3xl mt-4">{i.sem} Past Papers</h1>
                                    <div className="w-[80vw] flex flex-wrap gap-8 my-4 justify-center items-center">
                                        {i.subjects.map((e) => (
                                            <NavLink
                                                to={`/pastpapers/${CourseData.title}/${e.name}`}
                                                key={e.name}
